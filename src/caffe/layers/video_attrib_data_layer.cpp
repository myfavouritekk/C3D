...
#include "caffe/video_attrib_data_layer.hpp"
...
void* VideoAttribDataLayerPrefetch(void* layer_pointer) {
  CHECK(layer_pointer);
  VideoAttribDataLayer<Dtype>* layer = static_cast<VideoAttribDataLayer<Dtype>*>(layer_pointer);
  CHECK(layer);
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    if (!use_image){
      if (!use_temporal_jitter){
        read_status = ReadVideoToVolumeDatum(layer->file_list_[id].c_str(), layer->start_frm_list_[id],
              layer->attrib_label_list_[id], new_length, new_height, new_width, sampling_rate, &datum);
      }else{
    ...
    if (layer->output_labels_) {
      for (int label_idx = 0; label_idx < layer->num_attrib_; label_idx++) {
        int offset = item_id * layer->num_attrib_ + label_idx;
        top_label[offset] = datum.vector_label(label_idx);
      }
    }
...
template <typename Dtype>
VideoAttribDataLayer<Dtype>::~VideoAttribDataLayer<Dtype>() {
  JoinPrefetchThread();
}

template <typename Dtype>
void VideoAttribDataLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  string filename;
  int start_frm, label;
  infile >> this->num_attrib_;

  if ((!use_image) && use_temporal_jitter){
    while (infile >> filename) {
      file_list_.push_back(filename);
      vector<int> attributes;
      for (int i = 0; i < this->num_attrib_; i++) {
        infile >> label;
        attributes.push_back(label);
      }
      attrib_label_list_.push_back(attributes);
      shuffle_index_.push_back(count);
      count++;
    }
  } else {
    while (infile >> filename >> start_frm) {
      file_list_.push_back(filename);
  ...
  if (!use_image){
    if (use_temporal_jitter){
      srand (time(NULL));
      CHECK(ReadVideoToVolumeDatum(file_list_[0].c_str(), 0, attrib_label_list_[0],
                                 new_length, new_height, new_width, sampling_rate, &datum));
    }
    else
      CHECK(ReadVideoToVolumeDatum(file_list_[id].c_str(), start_frm_list_[id], attrib_label_list_[id],
                                   new_length, new_height, new_width, sampling_rate, &datum));
  }
  else{
    LOG(INFO) << "read video from " << file_list_[id].c_str();
    CHECK(ReadImageSequenceToVolumeDatum(file_list_[id].c_str(), start_frm_list_[id], attrib_label_list_[id],
                               new_length, new_height, new_width, sampling_rate, &datum));
  }
  ...
  // label
  if (output_labels_) {
    (*top)[1]->Reshape(this->layer_param_.image_data_param().batch_size(), this->num_attrib_, 1, 1, 1);
    prefetch_label_.reset(
        new Blob<Dtype>(this->layer_param_.image_data_param().batch_size(), this->num_attrib_, 1, 1, 1));
  }
...
template <typename Dtype>
void VideoAttribDataLayer<Dtype>::CreatePrefetchThread() {
  phase_ = Caffe::phase();
  ...
  // Create the thread.
  CHECK(!pthread_create(&thread_, NULL, VideoAttribDataLayerPrefetch<Dtype>,
        static_cast<void*>(this))) << "Pthread execution failed.";
}

template <typename Dtype>
void VideoAttribDataLayer<Dtype>::JoinPrefetchThread() {
  CHECK(!pthread_join(thread_, NULL)) << "Pthread joining failed.";
}

template <typename Dtype>
unsigned int VideoAttribDataLayer<Dtype>::PrefetchRand() {
  CHECK(prefetch_rng_);
...
template <typename Dtype>
Dtype VideoAttribDataLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top) {
...
INSTANTIATE_CLASS(VideoAttribDataLayer);
